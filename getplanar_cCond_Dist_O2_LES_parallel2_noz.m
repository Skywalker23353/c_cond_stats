function [z_axis,Avgf] = getplanar_cCond_Dist_O2_LES_parallel2_noz(wkdir,field_name,D,Na,Nc,R0,C_r,C_z,Ns,Ne,Y,c,delc)
...
%         f_idx
        %extract field
        A_f = get_burner_field_noz(wkdir,f_idx,field_name,NRa,NZa);
        C_f = get_overset_field(wkdir,f_idx,field_name,NZc);
        %interpolated plane
        C_F = squeeze(C_f(:,:,z_idx));
        A_F = squeeze(A_f(:,:,z_idx));
        
        A_f = get_burner_field_noz(wkdir,f_idx,'O2',NRa,NZa);
        C_f = get_overset_field_noz(wkdir,f_idx,'O2',NZc);
