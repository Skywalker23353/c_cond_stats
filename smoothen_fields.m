...
batch_smooth_fields('DataDir', data_dir, 'Fields', fields, 'PlotResults', plot_results_flag, ...
...
%                                           'PlotResults', true, 'BoundaryConfig', bc_config, 'SaveOutput', true);
    fields = {
%     'Temperature_smooth', struct('left', 800, 'right', 2319);
%     'density_smooth', struct('left', 0.425, 'right', 0.145);
%     'CH4_smooth', struct('left', 0.044, 'right', 0.0);
%     'O2_smooth', struct('left', 0.23, 'right', 0.041);
    'CO2_smooth', struct('left', 0.0, 'right', 0.105);
%     'H2O_smooth', struct('left', 0.0, 'right', 0.097);
    };

    % Process the fields
    process_fields_with_boundary_values('DataDir', data_dir, 'Fields', fields, ...
                                   'PlotResults', true, 'BoundaryWidth', 1, 'SaveOutput', true);
end
%%
fields = {
%          FILENAME,          LATEX NAME,                           FIG NAME, WINDOW, N_CYCLES
%         'Heatrelease_smooth_bc', '$\langle \dot{\omega}_{T}|c\rangle$', 3,3;%7, 3;
%         'Temperature_smooth_bc', '$\langle T|c\rangle$', 3,3;%9, 3;
%         'density_smooth_bc','$\langle \rho|c\rangle$', 3,3;%11, 3;
%         'CH4_smooth_bc','$\langle CH4|c\rangle$', 3,3;%11, 3;
        'CO2_smooth_bc','$\langle CO2|c\rangle$', 3, 1;
%         'H2O_smooth_bc','$\langle H2O|c\rangle$', 3, 3;
%         'O2_smooth_bc','$\langle O2|c\rangle$', 3, 3;
        };
